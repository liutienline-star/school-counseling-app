...
# --- 1. 核心安全與連線設定 (您的原始訴求：密碼、資料表、AI引擎完全維持) ---
AUTH_CODE = "1225"  
...
if not st.session_state.authenticated:
    ...
    with col_m:
        st.markdown("""
            <div style="text-align: center; background-color: #2e3440; padding: 40px; border-radius: 20px; border-top: 5px solid #88c0d0; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
                <h1 style="font-size: 50px;">🔐</h1>
                <h2 style="color: #88c0d0;">導師身分驗證</h2>
                <p style="color: #d8dee9;">請輸入授權碼進入系統</p>
            </div>
        """, unsafe_allow_html=True)
        ...
        if pwd_input:
            if pwd_input == AUTH_CODE:
                ...
                st.rerun() # 此處 rerun 位於主腳本分支，非 callback，不會報錯
            else:
                st.error("❌ 授權碼錯誤")
    st.stop()
